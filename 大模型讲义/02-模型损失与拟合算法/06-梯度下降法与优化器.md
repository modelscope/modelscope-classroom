...
其中 $\eta > 0$ 是学习率（Learning Rate），控制每步移动的幅度。

### 学习率的影响

学习率是最关键的超参数之一：

**学习率太大**：参数更新幅度过大，可能跳过最优点，导致震荡甚至发散。

**学习率太小**：收敛速度极慢，可能陷入局部最优或鞍点。
...
**物理直觉**：把优化想象成小球在损失函数曲面上滚动。动量使小球积累"惯性"，在一致的方向上加速，在震荡的方向上抵消。
...
Adam 是深度学习中最常用的优化器，对超参数不敏感，在大多数任务上表现良好。
...
在训练初期使用较小的学习率，逐渐增加到目标值。这有助于稳定训练初期的参数更新。
