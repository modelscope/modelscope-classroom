...
单个智能体的能力终究有限，当面对复杂任务时，多个智能体的协作往往能产生超越个体能力之和的效果。本节将介绍多智能体系统的设计模式、通信协议以及典型的协作架构。

## 为什么需要多智能体

### 单智能体的局限

即使是最强大的LLM，在处理复杂任务时也面临诸多挑战：
...
**专业化分工**：每个Agent专注特定任务，深度优于广度。

**并行处理**：多个Agent可以同时处理不同子任务。

**相互验证**：Agent间可以检查彼此的输出，提高质量。
...
一个主Agent负责任务分解和结果整合，多个从Agent执行具体任务：
...
当多个简单Agent按照简单规则交互时，可能产生复杂的群体行为：
...
多智能体系统的设计既是科学也是艺术。合理的角色分工、有效的通信机制、恰当的协调策略，是构建高效多智能体系统的关键。
