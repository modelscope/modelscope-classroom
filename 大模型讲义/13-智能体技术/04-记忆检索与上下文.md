...
智能体的"记忆"能力决定了它能否积累经验、保持对话连贯性以及处理需要长期追踪的复杂任务。本节将探讨智能体记忆系统的设计原理、实现方案以及与上下文管理的关系。

## 记忆的分类

借鉴认知科学的分类，智能体的记忆可以分为以下几种类型：

| 记忆类型 | 特点 | 智能体中的对应 |
|----------|------|----------------|
| 工作记忆 | 短期、容量有限 | 当前对话上下文 |
| 情景记忆 | 具体事件的记录 | 历史对话日志 |
| 语义记忆 | 一般性知识 | 知识库、向量存储 |
| 程序记忆 | 技能和习惯 | Prompt模板、工具定义 |

### 工作记忆

工作记忆对应LLM的上下文窗口，是智能体在单次交互中能够"看到"的所有信息：
...
LLM的上下文窗口有限（通常4K-128K tokens），需要精心管理：
...
记忆系统是智能体从"无状态对话机器人"进化为"有经验积累的助手"的关键。合理的记忆架构设计，能够让智能体在长期交互中展现出更强的个性化和一致性。
