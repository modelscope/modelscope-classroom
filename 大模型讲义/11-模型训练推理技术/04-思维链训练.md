...
模型的推理可能进入错误路径。思维回溯（Thought Backtracking）允许模型识别错误并重新开始。
...
在推理时投入更多计算以提升性能——"用更多的思考时间换取更好的答案"。
...
思维链训练和思维回溯技术代表了大模型能力提升的重要方向。通过让模型学会"思考"，而不仅仅是"回答"，可以显著提升其在复杂推理任务上的表现。这一领域仍在快速发展，新的方法和技术不断涌现。
