...
标准注意力的算术强度远低于这个阈值——大量时间花在读写中间结果 $\mathbf{S}$、$\mathbf{P}$ 上，而非实际计算。

## 7.1.2 Flash Attention 原理

### 核心思想

Flash Attention 的核心思想是**分块计算**（Tiling）：

1. 将 $\mathbf{Q}, \mathbf{K}, \mathbf{V}$ 分成小块
2. 每次只加载一块到 GPU 的高速缓存（SRAM）
3. 在 SRAM 中完成该块的全部计算
4. 将结果增量写回 HBM（显存）
5. **不存储完整的** $\mathbf{S}$ **和** $\mathbf{P}$
...
这带来了额外的计算开销，但节省的显存使得可以使用更大的批次或更长的序列，总体上是划算的。
