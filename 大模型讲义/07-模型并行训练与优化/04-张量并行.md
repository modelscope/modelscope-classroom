...
96 层 FFN：230 GB——**单层都超过了单卡显存**。

### FSDP 不够

FSDP 切分的是参数副本，但计算时仍需完整参数。当单层参数超过单卡显存时，FSDP 无能为力。

张量并行在**计算级别**切分，每个 GPU 只需要部分参数即可完成部分计算。

## 7.4.2 行切分与列切分

### 矩阵乘法切分

矩阵乘法 $\mathbf{Y} = \mathbf{X}\mathbf{W}$ 可以按两种方式切分：
...
这种"列-行"组合只需要**一次 AllReduce**（在 $\mathbf{W}_2$ 之后）。
...
当 TP 度数增加时，每次 AllReduce 的参与者增加，延迟上升。
