...
使用Denavit-Hartenberg（D-H）参数或指数积公式（Product of Exponentials）可以系统地建立各关节的变换矩阵。
...
其中 $\mathbf{J}^{\dagger}$ 是雅可比矩阵的伪逆。奇异位形附近需要使用阻尼最小二乘或其他正则化方法。

**动力学**描述力/力矩与运动之间的关系：

$$\mathbf{M}(\mathbf{q})\ddot{\mathbf{q}} + \mathbf{C}(\mathbf{q}, \dot{\mathbf{q}})\dot{\mathbf{q}} + \mathbf{g}(\mathbf{q}) = \boldsymbol{\tau}$$

其中 $\mathbf{M}$ 是质量矩阵，$\mathbf{C}$ 是科里奥利/离心力矩阵，$\mathbf{g}$ 是重力项，$\boldsymbol{\tau}$ 是关节力矩。

### 运动规划

运动规划的目标是生成从起点到目标的无碰撞轨迹。
...
抓取是机械臂操作的核心能力。传统方法基于几何分析计算力闭合抓取：
...
四足运动的核心挑战在于协调12个（每腿3自由度）或更多关节，同时保持动态平衡。
...
不同步态对应不同的相位差配置。trot步态中对角腿相位差为 $\pi$。
...
其中 $x$ 是质心水平位置，$p$ 是压力中心（ZMP），$z_c$ 是质心高度。
...
实时调整落足点是应对扰动的关键。Raibert控制器提供了直观的启发式：
...
机器人控制从经典的模型-规划范式向数据驱动的学习范式演进。无论是机械臂的灵巧操作、四足的敏捷运动，还是双足的动态平衡，强化学习与模仿学习都展现出强大的能力。大模型的引入进一步赋予机器人语言理解、常识推理、任务泛化的能力。随着硬件成本降低、仿真精度提升、迁移技术成熟，具身智能正在从实验室走向实际应用。
