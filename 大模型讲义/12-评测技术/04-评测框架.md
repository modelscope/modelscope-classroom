...
OpenCompass是由上海人工智能实验室开发的开源大模型评测平台，支持100+评测数据集和多种主流模型。

### 核心架构

OpenCompass采用模块化设计，主要包含以下组件：
...
EvalScope是阿里巴巴ModelScope团队开发的评测框架，与ModelScope生态深度集成，支持大模型和多模态模型评测。
...
VLMEvalKit专注于视觉语言模型（VLM）的评测，支持主流多模态模型和30+评测基准。
...
通过合理选择和组合评测框架，可以构建完整、高效的模型评测体系，为模型开发和选型提供全面的数据支撑。
