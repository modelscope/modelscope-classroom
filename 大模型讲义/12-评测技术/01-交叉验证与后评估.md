...
K折交叉验证就像一场歌剧的多场彩排。假设你有五幕戴，每次挑一幕当"观众彩排"（验证集），其余四幕照常排练（训练集）。五场彩排的平均评价就能很好地预测正式演出的效果。
...
**困惑度（Perplexity）**：语言模型的标准评估指标。直觉地说，困惑度衡量的是“模型有多“困惑””——如果模型对每个字都能准确预测，困惑度就低；如果每次都猕字不决，困惑度就高。一个困惑度为10的模型，大致相当于每次在约10个候选字中犹豫。
$$
