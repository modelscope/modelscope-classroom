...
想象一座图书馆的书架管理。传统方式是给每本书预留一整排书架，即使它只用了一半的位置，剩下的位置也不让其他书放。Paged Attention 则是把书架分成固定大小的格子，每本书可以分散存放在不同格子里，用一张索引卡片记录「第几章在哪个格子」——这样每个格子都能物尽其用，不再有空置浪费。
...
即使总空闲内存足够（512 + 剩余 Free），也因碎片无法使用。这像停车场里明明有很多空位，但全是零散的“半个车位”，一辆大车根本停不进去。

### 显存利用率

研究表明，传统方法的有效显存利用率只有 20-40%。大量显存被预分配但未使用，或因碎片而无法利用。
...
回到图书馆的例子：物理页就是书架上的格子，页表就是索引卡片。你不需要把一本书的所有章节放在连续的格子里，只要索引卡上记录清楚「第 3 章在 A 区第 7 格，第 4 章在 C 区第 2 格」，就能随时找到。
...
无需预分配，内存精确使用。这就像你在笔记本上写字，写满一页再翻到下一页，而不是一开始就准备 100 页空白纸“以防万一”。
...
CoW 使 Beam Search 的显存开销从 $O(k \cdot n)$ 降为 $O(n + k \cdot \text{新增})$。
...
Paged Attention 将显存利用率从 20-40% 提升到接近 100%（只有最后一个不满的块有浪费）。这就像从「给每本书预留一整排书架」变成了「按需分配格子」，空间利用率大幅提升。

设块大小 $B = 16$，最坏情况每个请求浪费 15 个 token 的空间，浪费率：

$$\text{浪费} < \frac{15}{平均序列长度}$$
