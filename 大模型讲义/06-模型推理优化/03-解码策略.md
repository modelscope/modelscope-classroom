...
其中 $z_x$ 是 logit，$\tau$ 是温度。你可以把温度理解为一个「冒险旋钮」：
...
Top-k 避免采样到概率极低的 token（“胡言乱语”），同时保持多样性。这就像你在餐厅点菜时，只看菜单上「热卖榜」前 k 名，而不会随机点一道从来没听说过的菜。
...
Top-p 相比 Top-k 更灵活：当分布集中时，候选集小；当分布分散时，候选集大。举个例子：当模型对下一个词非常确定时（比如「中华人民共和___」），候选集可能只有1个词；而当不确定性高时（比如「今天天气真___」），候选集可能有几十个词。这种自适应性是 Top-p 的精妙之处。
...
这就像写作文时的「草稿-审阅」模式：与其一个字一个字地斜酇，不如先快速写出一段草稿，然后让一个更有经验的人一次性审核。能用的留下，不行的划掉重写。
