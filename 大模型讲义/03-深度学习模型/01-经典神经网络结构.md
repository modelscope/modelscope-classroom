...
本节回顾深度学习发展历程中的三类经典网络：全连接网络（DNN）、卷积神经网络（CNN）和长短期记忆网络（LSTM）。虽然 Transformer 已成为主流架构，但理解这些经典结构有助于把握深度学习的核心思想，也能更好地理解 Transformer 为何要那样设计。
...
这个定理说明神经网络具有足够的表达能力。但它没有告诉我们需要多宽的网络、如何找到合适的参数。实践中，深度网络（多层）往往比宽度网络（单层但神经元数量巨大）更有效——这涉及到表示学习的层次性。
...
对于处理图像的任务，若输入是 $224 \times 224 \times 3$ 的图片，第一层就需要 $224 \times 224 \times 3 \times d_1 \approx 1.5 \times 10^5 \cdot d_1$ 个参数。当隐藏层维度 $d_1 = 1000$ 时，仅第一层就有 1.5 亿参数。这种参数量在早期是不可接受的。
...
卷积神经网络（Convolutional Neural Network, CNN）利用卷积操作提取局部特征，通过参数共享大幅减少参数量。
...
例如，5 层 $3 \times 3$ 卷积的感受野是 $1 + 5 \times 2 = 11$。这解释了为什么需要堆叠多层小卷积核而非使用单层大卷积核——参数量更少，非线性更多，感受野相同。
...
序列数据（文本、语音、时间序列）具有时序依赖性。循环神经网络（Recurrent Neural Network, RNN）通过隐藏状态传递历史信息。
...
这导致基础 RNN 难以学习长程依赖：模型很难将 100 步之前的信息传递到当前时刻。

### LSTM：门控机制

长短期记忆网络（Long Short-Term Memory, LSTM）由 Hochreiter 和 Schmidhuber 于 1997 年提出，通过**门控机制**解决梯度消失问题。

LSTM 引入**细胞状态** $\mathbf{c}_t$（cell state）作为信息的"传送带"，通过三个门控制信息流动：
...
**顺序计算**：$\mathbf{h}_t$ 依赖 $\mathbf{h}_{t-1}$，无法并行化。长序列的训练速度受限于 GPU 的时序计算。

**长程依赖的"遗忘"**：虽然理论上 LSTM 可以保持长程信息，但实践中，细胞状态在数百步后仍会"稀释"。
