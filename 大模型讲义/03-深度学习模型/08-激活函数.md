...
igmoid 曾是神经网络的默认激活，但因梯度饱和问题已很少在隐藏层使用。现在主要用于：
- 二分类输出层
- 门控机制（LSTM、GLU）
...
eLU 在 2012 年 AlexNet 中大放异彩，成为深度学习的默认激活。但在 Transformer 中，ReLU 逐渐被 GELU 和 SwiGLU 取代。
...
��觉：以概率 $\Phi(z)$ 保留输入 $z$，概率 $1 - \Phi(z)$ 将其置零。当 $z$ 很大时，几乎全部保留；当 $z$ 很小（负）时，几乎全部置零。这比 ReLU 的硬阈值更平滑。
