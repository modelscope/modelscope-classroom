...
其中 $y_i$ 是 one-hot 标签。这个简洁的梯度使得训练稳定高效。
...
直觉：以概率 $\Phi(z)$ 保留输入 $z$，概率 $1 - \Phi(z)$ 将其置零。当 $z$ 很大时，几乎全部保留；当 $z$ 很小（负）时，几乎全部置零。这比 ReLU 的硬阈值更平滑。
...
这种选择性激活提供了更灵活的计算路径。直觉上，不同的输入可能需要激活不同的"专家"特征；门控机制允许这种动态路由。
...
激活函数的选择看似细节，但在大规模训练中，微小的性能差异会被放大。SwiGLU 相比 ReLU 的提升虽然不大（可能 1-2%），但在百亿参数、万亿 token 的训练中，这个提升意义重大。
