...
**完全基于注意力**：序列内任意两个位置可以直接交互，无需逐步传递。在翻译局的比喻中，这意味着句子开头的词可以直接和句尾的词“对话”，不必经过中间所有词的“传话”。
...
**实践发现**：Pre-Norm 在深层网络中更稳定。直观理解：Pre-Norm 的残差分支直接是 $f(\cdot)$，而 Post-Norm 的残差分支是 $\text{LayerNorm}(\mathbf{x} + f(\mathbf{x})) - \mathbf{x}$，后者的梯度流更复杂。这就像每次开会前先把议题整理清楚（Pre-Norm），还是开完会再整理会议纪要（Post-Norm）——前者确保每次讨论都基于清晰的输入，流程更可控。
