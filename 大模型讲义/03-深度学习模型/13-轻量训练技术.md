...
其中 $m$ 是幅度标量，$\mathbf{V}$ 是方向矩阵。LoRA 只应用于方向部分，幅度单独学习。
...
QLoRA 可以在单张 24GB 显卡上微调 65B 模型，是资源受限场景的利器。
...
其中 $Q$ 是量化函数，$D$ 是反量化函数。前向用量化权重，反向用直通估计（Straight-Through Estimator）更新全精度权重。
...
对于 70B 模型、4096 序列长度、FP16，激活显存可达数十 GB。
